...
int main() {
    ...
    try {
        ...
        // 创建缓冲区
        cl::Buffer bufferA1(context1, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, a.data());
        cl::Buffer bufferB1(context1, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, b.data());
        cl::Buffer bufferC1(context1, CL_MEM_WRITE_ONLY, sizeof(float) * n);
        cl::Kernel kernel1(program1, "vectorSub");
        printf("[Check] CL_MEM_COPY_HOST_PTR cl::Buffer bufferA %p\n", bufferA1);
        printf("[Check] CL_MEM_COPY_HOST_PTR cl::Buffer bufferC %p\n", bufferC1);
        ...
        // 3. check in-place kernel: access 2 buffs on different devices: ok
        // 3. check in-place kernel: access 2 buffs on different contexts: failed
        cl::Kernel kernel2(program0, "vectorAdd");
        kernel2.setArg(0, bufferC);
        kernel2.setArg(1, bufferC);
        kernel2.setArg(2, bufferC);
        kernel2.setArg(3, n);
        queue0.enqueueNDRangeKernel(kernel2, cl::NullRange, global, cl::NullRange);
        // queue1.enqueueReadBuffer(bufferC1, CL_TRUE, 0, copy_len, c.data());
